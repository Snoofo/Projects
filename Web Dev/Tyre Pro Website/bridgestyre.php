        ...
        <div class="tyre"><!-- Image sourced from www.continental.co.uk for educational purposes only -->
          <div class="prodmain" style="background-image: url(bridgestyre.png); background-size: cover">
            <h3>Bridgestone Potenza</h3>
            <h3>In Stock:</h3>
            <p><?php
              if (!isset($_SESSION['user'])){
                $price = $_SESSION['products']['bstone'];
                echo "$".number_format($price, 2, '.', '');
              }
              else {
                $price = ($_SESSION['products']['bstone'] * (1-(0.01 * $_SESSION['user']['discountP3'])));
                echo "$".number_format($price, 2, '.', '');
              }
            ?></p>
          </div>
          ...
          document.getElementById('total').value = ((quantity.value)*(price.value)).toFixed(2)">
            ...
            <input type="hidden" name="productID" value="bridestyre">
